...
function Logger.new(config: LoggerConfig | string, IsEnabled: boolean?)
	...
	if typeof(config) == "string" then
		...
		if IsEnabled ~= nil then
			...
			self._isEnabled = true -- Default to true
		end
	else
		self._moduleName = config.moduleName
		self._minLevel = config.minLevel or (IsStudio and "DEBUG" or "INFO")
		if config.includeTimestamp ~= nil then
			self._includeTimestamp = config.includeTimestamp
		else
			self._includeTimestamp = true
		end
		if config.includeStackTrace ~= nil then
			self._includeStackTrace = config.includeStackTrace
		else
			self._includeStackTrace = true
		end
		self._prefix = config.prefix or ""
		self._outputFunction = config.outputFunction
		if config.isEnabled ~= nil then
			self._isEnabled = config.isEnabled
		else
			self._isEnabled = true -- Default to true
		end
	end
