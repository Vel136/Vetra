...
function SimulateCast.StepProjectile(Solver: any,Cast: any,Delta: number,IsSubSegment: boolean)
	...
	-- ── Drag + Magnus + 6DOF combined recalc ────────────────────────────────
	-- All interval-based forces are evaluated together so none can be
	-- accidentally omitted when another is active. Wind and GyroDrift are
	...
	if ShouldRecalculate and (NeedsDrag or NeedsMagnus or Needs6DOF) then
		if not BaseAcceleration then
			...
			-- [6DOF] Step angular state first so lift and AoA-drag are computed
			-- from the current orientation before being composited below.
			...
			if NeedsDrag then
				...
				-- and by 6DOF AoA-dependent drag multiplier (1.0 when 6DOF disabled).
			...
			if NeedsMagnus then
				if Needs6DOF then
					-- 6DOF owns the angular state. Derive the effective spin
					-- vector from Runtime.AngularVelocity's axial component so
					-- Magnus force and gyroscopic precession share the same spin
					-- value. SpinDecay is skipped — 6DOF RollDampingCoeff already
					-- handles spin decay through roll damping.
	...
	if Runtime.CosmeticBulletObject then
		-- [6DOF] When 6DOF is active, orientation is authoritative — use it
		...
		if not PierceResolved then
			...
			if BounceConditionsMet then
				...
				if not IsCornerTrapped then
					...
					FinalVelocity = BouncePhysics.ApplyRestitution(FinalVelocity, EffectiveNormal, BaseRestitution, MaterialMultiplier, NormalPerturbationAmount)

					local PostBounceOrigin = RaycastResult.Position + EffectiveNormal * Constants.NUDGE

					-- [6DOF] Reflect body orientation and dampen roll on bounce.
					if SixDOFPhysics.IsEnabled(Behavior) then
