...
function Fragmentation.SpawnFragments(
	...
	for Index = 1, FragmentCount do
		...
		local ParentParams = Behavior.RaycastParams
		local FreshRaycastParams = RaycastParams.new()
		FreshRaycastParams.FilterType = ParentParams.FilterType
		local FilterSnapshot = table.clone(Behavior.OriginalFilter)
		if ParentParams.IncludeInstances ~= nil then
			FreshRaycastParams.IncludeInstances = FilterSnapshot
		elseif ParentParams.ExcludeInstances ~= nil then
			FreshRaycastParams.ExcludeInstances = FilterSnapshot
		else
			FreshRaycastParams.FilterDescendantsInstances = FilterSnapshot
		end
...
return table.freeze(Fragmentation)
