--[=[
	...
	each cast using the exact kinematic formula `P(t) = Origin + V0t + 1/2At^2`,
...
--[=[
	...
	`SetCoriolisConfig()`, `GetSignals()`, and `Destroy()` all behave the same way.

	```lua
	local Solver = Vetra.newParallel({
	    ShardCount = 6,
	    SpatialPartition = { FallbackRate = 4 },
	})
...
--[=[
	...
	**Signal contracts:**
	...
	| `OnTerminated` | `context`, `reason: TerminationReason` | Fires on any termination cause, including manual `Terminate()` (`reason = "manual"`). |
	| `OnPreTermination` | `context`, `reason: TerminationReason`, `mutate: MutateFn` | Before cleanup, for simulation-driven terminations only (not manual `Terminate()`). Call `mutate(cancelled: boolean, newReason?)` to cancel. Per-reason 3-strike limit applies. |
	...
	**`OnPreTermination` 3-strike rule:**
	...
	`OnPreTermination` fires only for simulation-driven terminations. A manual
	`Cast:Terminate()` / `BulletContext:Terminate()` skips the hook entirely and cannot be
	cancelled, so an explicit termination request is always honoured.
...
--[=[
	...
	1 frame in `LODInterval` (default every 3rd), reducing raycast cost.
...
--[=[
	...
	**solver-level environment property**, it is not configurable per-bullet
	...
	The Omega vector is precomputed here and cached on the solver so no
	...
	**Scale reference:**
	...
	@param scale number -- Exaggeration multiplier on Earth's actual omega. `0` = disabled.
]=]
