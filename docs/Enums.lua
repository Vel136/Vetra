...
--[=[
	...
	**Analytic models**, mathematically defined drag curves:
	...
	|-----|-------|-------------|
	| `Quadratic` | `1` | Deceleration proportional to speed^2, default; most accurate for subsonic bullets |
	| `Linear` | `2` | Deceleration proportional to speed |
...
--[=[
	...
	Reason strings identifying why a cast terminated. Compare against these rather
	than hardcoding raw strings, if a value is ever renamed, every reference site
	produces a detectable nil rather than silently passing the wrong string.

	`OnTerminated` receives the reason as its second argument, for every value below.
	`OnPreTermination` also receives it, but sees every value **except** `Manual`, which is
	only produced by the manual `Terminate()` path that skips the hook.
	...
	| `Manual` | `"manual"` | `VetraCast:Terminate()` was called, or the solver was destroyed. Never seen by `OnPreTermination` |
