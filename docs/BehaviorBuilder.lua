--[=[
	...
	**Minimum required fields when `:SixDOF():Enabled(true)`:**
	...
	| `ReferenceArea` | Cross-sectional area in studs^2, scales all aero forces |
	...
	The solver computes precession as `omega_prec = spinAxis x aeroTorque / H`
	...
	**Lift**, `LiftCoefficientSlope` (dCL/dalpha) scales the Magnus-like lift
	force proportional to AoA. Typical range `1.0`,`4.0`. Set to `0` to
	disable lift entirely and model drag-only nose attitude.

	**AoA-dependent drag**, `AoADragFactor` multiplies drag by `1 + k*sin^2(AoA)`.
	...
	**Reference values for a typical rifle bullet:**
	- `ReferenceArea` ~= `0.005`,`0.02` studs^2
...
--[=[
	...
	```lua
	-- A frozen behavior from a config file, preset, or another module
	local existing = BehaviorBuilder.Sniper():Build()
