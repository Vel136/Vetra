...
Yes. All 6DOF computation is pure math — no Instance access, no cross-Actor callbacks.
`InitialOrientation` and `InitialAngularVelocity` are passed by value and serialized correctly.
There is no parallel-specific penalty beyond the extra math per step.
...
The main simplification relative to a full BRL model is that aerodynamic coefficients are treated
as linear with AoA rather than using Mach-indexed lookup tables. This is the correct approximation
for small-angle flight, which covers the vast majority of game projectile trajectories. For
ballistic research or extreme-range simulation, a dedicated exterior ballistics tool is more
appropriate.
