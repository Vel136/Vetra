...
--[=[
	...
	kinematic formula `P(t) = Origin + V0t + 1/2At^2`. Always accurate to the
...
--[=[
	Suspends simulation for the next `frames` frames. Unlike [VetraCast:Pause],
	the frame deltas skipped while suspended are accumulated and released into a
	single catch-up step once the suspension ends, so the bullet advances in
	world time exactly as if it had never been suspended, it simply arrives
	there in one jump rather than incrementally.

	The frame budget counts down independently of [VetraCast:SuspendTime];
	if both are active the cast stays suspended until both reach zero. Repeated
	calls add to the remaining frame budget.

	:::warning Tunneling
	The catch-up step advances by the full accumulated delta in one raycast
	segment. A fast bullet suspended for many frames may skip thin geometry and
	under-sample curved trajectories, the same tradeoff [LOD] accepts. Prefer
	short suspensions for fast projectiles.
	:::

	@param frames number -- number of frames to skip
]=]
function VetraCast:SuspendFrame(frames: number) end

--[=[
	Suspends simulation for the next `seconds` of accumulated frame time. Behaves
	like [VetraCast:SuspendFrame] but counts down a time budget rather than a
	frame count; the skipped deltas are accumulated and released in a single
	catch-up step once the suspension ends.

	The time budget counts down independently of [VetraCast:SuspendFrame]; if
	both are active the cast stays suspended until both reach zero. Repeated calls
	add to the remaining time budget.

	@param seconds number -- seconds of simulation time to skip
]=]
function VetraCast:SuspendTime(seconds: number) end

--[=[
	Clears any active frame and time suspension, discarding both remaining
	budgets. Any deltas accumulated so far are released into the next step.
]=]
function VetraCast:ClearSuspend() end

--[=[
	Returns whether this cast currently has a frame or time suspension active.

	@return boolean
]=]
function VetraCast:IsSuspended(): boolean end
...
--[=[
	...
	Fires `OnTerminated` with `reason = TerminateReason.Manual`. Calling `Terminate`
	on an already-dead cast is a no-op.

	:::note Not cancellable
	`OnPreTermination` does **not** fire for a manual `Terminate()`, so handlers cannot
	cancel it or rewrite its reason. The cast dies with reason `Manual`. Only
	simulation-driven terminations (hit, distance, speed, and so on) are cancellable.
	:::
]=]
