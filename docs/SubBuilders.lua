...
--[=[
	...
	Any `Vector3` is accepted and used as given: `Vector3.zero` disables drop entirely,
	and upward or sideways vectors are applied as specified.
...
--[=[
	Lateral drift rate in **1/s**, an acceleration *per unit speed*: the applied
	acceleration is `value * speed`. Drift scales with velocity, so lateral displacement
	grows superlinearly with time of flight.

	Typical values are small, on the order of `1e-3`. Setting this enables drift.
...
--[=[
	Chaotic lateral acceleration magnitude in studs/s^2 applied while tumbling.
...
--[=[
	dCL/dalpha, lift coefficient slope. Scales aerodynamic lift with angle of attack.
...
--[=[
	dCm/dalpha, pitching moment slope. Negative values produce a statically stable
...
--[=[
	sin^2alpha drag multiplier. Adds extra drag proportional to angle of attack squared.
	`3.0` triples drag at 90 deg AoA. `0` = no AoA-dependent drag.
...
--[=[
	Reference cross-sectional area in studs^2. Scales all aerodynamic forces.
...
--[=[
	Air density in kg/m^3. Scales all aerodynamic forces.
...
--[=[
	Mach-indexed CLalpha table. Overrides [SixDOFBuilder:LiftCoefficientSlope] when set.
...
--[=[
	Mach-indexed Cmalpha table. Overrides [SixDOFBuilder:PitchingMomentSlope] when set.
...
--[=[
	...
	Sub-builder for LOD configuration, `Distance` (when a bullet throttles) and
	`Interval` (how hard). Opened via [BehaviorBuilder:LOD].
	Call `:Done()` to return to the root [BehaviorBuilder].
]=]
local LODBuilder = {}
...
--[=[
	Once the bullet is beyond `Distance`, step 1 frame in `value`.

	The skipped time is banked and released into the catch-up step, so the
	flight path is unchanged, only its resolution is. Must be `>= 1`, where
	`1` means "never throttle". Non-integers are floored.
...
function LODBuilder:Interval(value: number): LODBuilder end
