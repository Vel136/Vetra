...
--[=[
	...
	**Physics**
	...
	.Gravity Vector3? -- Gravitational acceleration (negative Y = downward). Any vector is used as given, including `Vector3.zero`. Default: workspace gravity downward.
	...
	**Gyroscopic Drift**
	.GyroDriftRate number? -- Lateral drift rate in 1/s, an acceleration per unit speed (applied accel is `Rate * speed`). `nil` = disabled. Default: `nil`
	...
	**Tumble**
	...
	.TumbleLateralStrength number? -- Chaotic lateral acceleration magnitude in studs/s^2. Default: `0`
	...
	**LOD**
	...
	.LODInterval number? -- Once beyond `LODDistance`, step 1 frame in N. Must be `>= 1`, where `1` disables throttling. Skipped time is banked and released into the catch-up step, so the flight path is unchanged. Default: `3`
	...
	**Cosmetic**
	...
	.CosmeticBulletNonQueryable boolean? -- When `true`, sets `CanQuery = false` and `CanTouch = false` on the cloned cosmetic bullet so it is invisible to raycasts and touch events. Applies to `CosmeticBulletTemplate` clones only, not `CosmeticBulletProvider` parts. Default: `true`
...
--[=[
	@type DragModel "Linear" | "Quadratic" | "G1" | "G2" | "G3" | "G4" | "G5" | "G6" | "G7" | "G8" | "GL" | "Custom"
	@within TypeDefinitions

	Drag model used to compute aerodynamic deceleration each `DragSegmentInterval`.

	**Analytic models:**
	- `"Linear"`, deceleration proportional to speed.
	- `"Quadratic"`, deceleration proportional to speed^2 (default). Most physically accurate for subsonic bullets.
...
--[=[
	@type TerminationReason "hit" | "distance" | "displacement" | "speed" | "corner_trap" | "manual"
	@within TypeDefinitions

	The reason a cast was terminated. Passed as the second argument to
	`OnPreTermination` signal handlers. Prefer `Vetra.Enums.TerminateReason` over raw strings.

	- `"hit"`, bullet struck a surface and was not pierced or bounced.
	- `"distance"`, `MaxDistance` was reached.
	- `"displacement"`, `MaxDisplacement` (straight-line distance from origin) was reached.
...
--[=[
	...
	.HotRadius number? -- Inner ring radius around an interest point, in **cells** (not studs), stepped every frame. Default: `1`
	.WarmRadius number? -- Outer ring radius in **cells** (not studs), stepped every 2nd frame. Must be >= HotRadius, clamped up if smaller. Default: `3`
	.Tiers { { Radius: number, Rate: number } }? -- Explicit ring list, replaces HotRadius/WarmRadius. `Radius` is in cells, `Rate` is "step 1 frame in N". Sorted by radius internally. Default: `nil` (use HotRadius/WarmRadius)
	.FallbackRate number? -- Step rate for cells outside every ring, as a **number**, not a string. `1` = every frame, `4` = every 4th. Also accepted under the legacy name `FallbackTier`. Default: `1`
